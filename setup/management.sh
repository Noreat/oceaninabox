...
apt_install duplicity python3-gnupg python3-pip python3-venv certbot rsync

# b2sdk is used for backblaze backups.
# boto3 is used for amazon aws backups.
# Both are installed outside the pipenv, so they can be used by duplicity
hide_output pip_install_system --upgrade b2sdk boto3

# Create a virtual environment for the installation of Python 3 packages
...
hide_output $venv/bin/pip install --upgrade \
	...
	qrcode[pil] pyotp \
	"idna>=2.0.0" "cryptography==42.0.8" psutil postfix-mta-sts-resolver \
	b2sdk boto3
